...
Mobi est un chatbot en JavaScript natif, sans framework, spécialisé dans l'aide à la préparation de trajets en Île-de-France. Il répond avec un cerveau à règles et présente une identité composée d'un nom, d'un emoji, d'un message d'accueil et de trois suggestions.

Fichiers principaux :

- `public/js/brain.js` : fonctions pures `validateMessage` et `replyTo`, aucun accès à la page ;
- `public/js/persona.js` : identité de Mobi et fonction validatePersona ;
...
- Ne jamais ajouter une autre identité au chatbot : l'identité est Mobi 🚇.
- Ne jamais ajouter d'informations en temps réel sur les transports : Mobi renvoie vers l'application officielle.
- Ne jamais demander de données personnelles à l'utilisateur.
